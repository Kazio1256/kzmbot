...
if((preg_match('/^discountCustomPlanDay(\d+)/',$userInfo['step'], $match) || preg_match('/enterCustomPlanName(\d+)_(\d+)_(\d+)_(\d+)_(?<buyType>\w+)/',$userInfo['step'], $match)) && ($botState['sellState']=="on" ||$from_id ==$admin) && $text != $buttonValues['cancel']){
    ...
    if(!preg_match('/^discountCustomPlanDay/', $userInfo['step'])) $keyboard[] = [['text' => "💫کد تخفیف دارم",  'callback_data' => "haveDiscountCustom_" . $rowId]];
	$keyboard[] = [['text' => $buttonValues['cancel'], 'callback_data' => "mainMenu"]];
    $price = ($price == 0) ? 'رایگان' : number_format($price).' تومان ';
...
if((preg_match('/^discountSelectPlan(\d+)_(\d+)_(\d+)/',$userInfo['step'],$match) || 
    ...
    if($price == 0 or ($from_id == $admin)){
        ...
        if(!preg_match('/^discountSelectPlan/', $userInfo['step'])) $keyboard[] = [['text' => "💫کد تخفیف دارم",  'callback_data' => "haveDiscountSelectPlan_" . $match[1] . "_" . $match[2] . "_" . $rowId]];
...
if($data == 'reciveApplications') {
    ...
    $keyboard[] = ['text'=>$buttonValues['back_to_main'],'callback_data'=>"mainMenu"];
    $keyboard = array_chunk($keyboard,1); 
    editText($message_id, "1. سیستم عامل خود را از بین برنامه های پایین انتخاب کنید .
2. بعد از دانلود و نصب برنامه لینک 🌐 subscription خود را کپی کنید (یک بار روی لینک کلیک کنید کپی میشه ) .
3. وارد برنامه Happ Proxy شوید روی به علامت اضافه ➕ بالای تصویر و سپس گزینه دوم کلیک کنید .
4. چندین سرور وارد برنامه میشن طلایی، ترکیه، انگستان و... یک بار روی سرور مورد نظر کلیک کنید و سپس دکمه روشن شدن وی پی ان رو بزنید.

نکته : اگر اشتراک ها وارد نشدن تمام وی پی ان های روی دستگاهتون رو خاموش و مجدد تلاش کنید ، اگر باز هم نشد اینترنتتون رو تغییر بدید و درنهایت اگر موفق نشدید به آیدی پشتیبان پیام بدید .

کانال آموزش تصویری و فایل نصبی برنامه ها : 👇🏻

➡️ @v2rayngtelteach ⬅️", json_encode(['inline_keyboard'=>$keyboard]));
}
